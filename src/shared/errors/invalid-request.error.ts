...
export class InvalidRequestException extends HttpException {
  private static walk(root: string, error: ValidationError): Array<ValidationErrorResponse> {
    ...
    return flatten(map(error.children, (child) => InvalidRequestException.walk(path, child)));
  }
